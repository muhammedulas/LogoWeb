...
export class SalesExpensesService {
  ...
  getRecords(lim: number, offset: number, q?: string) {
    ...
    else return this.http.get<salesExpenseResp>(this.rootUrl + "/api/v1/salesExpenses?offset=" + offset + "&limit=" + lim + "&q=(CODE like '*" + q + "*' or NAME like '*" + q + "*')" + "&withCount=true", { headers })
  }
