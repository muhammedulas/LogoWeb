...
export class ChequeAndPnotesService {
  ...
  getCPs(lim:number,offset:number,q?:string){
    ...
    else return this.http.get<chequeAndPnoteResp>(this.rootUrl + "/api/v1/chequeAndPnotes?offset=" + offset + "&limit=" + lim + "&q=(NUMBER like '*" + q + "*' or OWING like '*" + q + "*' or NEW_SERIAL_NO like '*" + q + "*' or GUARANTOR like '*" + q + "*')" + "&withCount=true", { headers }) 
  }
