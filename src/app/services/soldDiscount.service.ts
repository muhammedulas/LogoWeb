...
export class SoldDiscountService {
  ...
  getRecords(lim: number, offset: number, q?: string) {
    ...
    else return this.http.get<soldDiscountResp>(this.rootUrl + "/api/v1/salesDiscounts?offset=" + offset + "&limit=" + lim + "&q=(CODE like '*" + q + "*' or DESCRIPTION like '*" + q + "*' or VAT_PERC like '*" + q + "*')" + "&withCount=true", { headers })
  }
