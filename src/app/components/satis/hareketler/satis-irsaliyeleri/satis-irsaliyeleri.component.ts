...
export class SatisIrsaliyeleriComponent implements OnInit {
  ...
  add() {
    if(this.rightsService.checkRight(4091) == false){
  ...
  delete(id: number) {
    if(this.rightsService.checkRight(4093) == false){
      this.tstUnAuthorized2()
      return
    }
  ...
  edit_inspect(inspectMode: boolean) {
    if(inspectMode){
      if(this.rightsService.checkRight(4094) == false){
        this.tstUnAuthorized2()
        return
      }
    }
    else {
      if(this.rightsService.checkRight(4092) == false){
        this.tstUnAuthorized2()
        return
      }
    }
