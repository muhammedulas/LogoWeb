...
import { inwardProccessingPermit } from 'src/app/models/inwardProcessingPermit';
import { InwardProcessingPermitsService } from 'src/app/services/inwardProccessingPermits.service';

@Component({
  selector: 'app-diib',
  templateUrl: './diib.component.html',
  styleUrls: ['./diib.component.scss']
})
export class DiibComponent implements OnInit {

  constructor(
    private toast: ToastrService,
    private router: Router,
    private service: InwardProcessingPermitsService,
  ...
  public selectedRecord: inwardProccessingPermit = new inwardProccessingPermit();
  public dataSet: inwardProccessingPermit[] = []
  private errorMsg: string = "";
  private errorCode: string = "";
  public loaded: boolean = false;
  public displayedColumns: string[] = ["FicheNo", "BegDate", "EndDate", "Status", "DocNum", ]
  ...
  select(rec: inwardProccessingPermit) {
  ...
  setRecLimit(newLim: number) {
    ...
    this.getAllRecords(0)
    this.pageCount = Math.floor(this.itemCount / this.recLimit)
    this.router.navigate(['/ihracat/diib'])
  }
