...
export class HizmetAlimFiyatlariComponent implements OnInit {
  ...
  add(){
    if(this.rightsService.checkRight(3041) == false){
  ...
  delete(id:number) {
    if(this.rightsService.checkRight(3043) == false){
      this.tstUnAuthorized2()
      return
    }
  ...
  edit_inspect(inspectMode:boolean){
    if(inspectMode){
      if(this.rightsService.checkRight(3044) == false){
        this.tstUnAuthorized2()
        return
      }
    }
    else {
      if(this.rightsService.checkRight(3042) == false){
        this.tstUnAuthorized2()
        return
      }
    }
